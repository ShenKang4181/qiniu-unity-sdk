...
namespace Qiniu.CDN
{
	public class FluxResult : HttpResult
	{
		public FluxInfo Result
		{
			get
			{
				FluxInfo result = null;
				if (base.Code == 200 && !string.IsNullOrEmpty(base.Text))
				{
					result = JsonConvert.DeserializeObject<FluxInfo>(base.Text);
				}
				return result;
			}
		}
		...
		public override string ToString()
		{
			...
			if (Result != null)
			{
				...
				{
					stringBuilder.Append("flux:");
					foreach (KeyValuePair<string, FluxInfo.FluxData> datum in Result.Data)
					{
