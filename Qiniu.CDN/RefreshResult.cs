...
namespace Qiniu.CDN
{
	public class RefreshResult : HttpResult
	{
		public RefreshInfo Result
		{
			get
			{
				RefreshInfo result = null;
				if (base.Code == 200 && !string.IsNullOrEmpty(base.Text))
				{
					result = JsonConvert.DeserializeObject<RefreshInfo>(base.Text);
				}
				return result;
			}
		}
		...
		public override string ToString()
		{
			...
			if (Result != null)
			{
				...
				stringBuilder.AppendFormat("dirSurplusDay:{0}\n", Result.DirSurplusDay);
				stringBuilder.AppendFormat("urlQuotaDay:{0}\n", Result.UrlQuotaDay);
				stringBuilder.AppendFormat("urlSurplusDay:{0}\n", Result.UrlSurplusDay);
			}
